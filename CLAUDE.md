...
**Three evaluations spend money and one does not.** `make test` (1112 tests) is offline and free. `evaluation/judge_eval.py`, `evaluation/ragas_eval.py` and `evaluation/injection_eval.py` all call real models; `ragas_eval table` and `injection_eval table` reprint the saved run for free. `make smoke` boots both servers and uses the app for real (~$0.006) — it reuses servers it finds already running and stops only what it started.
...
- `make test` / `make lint` — the offline suite (1112 tests) / deps-check + ruff + eslint. CI (`.github/workflows/ci.yml`) runs the same targets plus the frontend typecheck and production build.
