...
**The headline WP7 result is uncomfortable and stays in the README.** On the 25-question golden set the **naive top-k baseline beats the full pipeline** on every metric (hit rate 1.000 vs 0.920). The set is lookup-shaped — each question written *from* one requirement, so BM25 alone nails it — and contains none of the enumeration questions expansion and rerank exist for. Do not "fix" this by rewriting the golden set to flatter the pipeline; if you extend it, add enumeration questions *alongside* and report both.
