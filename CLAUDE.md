...
**Three evaluations spend money and one does not.** `make test` (1109 tests) is offline and free. `evaluation/judge_eval.py`, `evaluation/ragas_eval.py` and `evaluation/injection_eval.py` all call real models; `ragas_eval table` and `injection_eval table` reprint the saved run for free. `make smoke` boots both servers and uses the app for real (~$0.006) — it reuses servers it finds already running and stops only what it started.
...
- `make smoke` — the end-to-end test (F7.1): boots whatever is not already running, asks three real questions **through the frontend proxy**, runs a scoped report to completion, deletes its own threads, and stops only the servers it started. **Spends money** (~$0.006). Exits 0/1.
